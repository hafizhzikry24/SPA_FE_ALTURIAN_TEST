<template>
    <div v-if="!comments.length" class="text-center text-gray-500">No comments yet.</div>
      ...
      <div v-for="comment in comments" :key="comment.id" class="bg-gray-50 p-4 rounded-lg">
        <div v-if="editingCommentId === comment.id">
          ...
          <div class="mt-2 flex justify-end space-x-2">
            <button @click="saveEdit(comment)" class="text-blue-500 hover:underline">Save</button>
        ...
        <div v-else>
          ...
          <div class="flex justify-between items-center">
            ...
            <div v-if="comment.author.id === currentUser?.id" class="flex space-x-2 items-center">

              <PencilIcon @click="startEdit(comment)" class="text-blue-500 hover:underline size-4">Edit</PencilIcon>
              <TrashIcon @click="deleteComment(comment.id)" class="size-4 text-red-500 hover:underline">Delete</TrashIcon>
            </div>
          </div>
        </div>
      </div>
